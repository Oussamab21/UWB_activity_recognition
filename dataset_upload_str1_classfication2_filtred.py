        ...
        for i in (dataset_files):      ##### liste of files in every activity 
            ...
            for j in (list_files_current_activity):
                ...
                #with open(list_files_current_activity[j]) as file:
                    ...
                    #wc = fp / (0.5*Fs)
                    #wc=  [1,5]
                    #sos=signal.cheby1(filter_order, Ap,wc , 'bandpass',analog=False,output='sos')#,fs=50)
                    
                    
                    sos = signal.cheby1(filter_order, 0.025,[0.12, 5],btype=filter_type, output='sos', fs=Fs, analog=False)
                    
                    
                    ## shaping the np scaterring matrix
                    data_UWB=np.array(data_UWB)
                    data_UWB=data_UWB.transpose()
                    ...
                    #### filtering
                    data_UWB=signal.sosfilt(sos,data_UWB,axis=1)
                    
                    
                    data_UWB=data_UWB.transpose()
                    data_UWB=data_UWB.tolist()
                   
            
                    
                    #data_UWB1=filt_data_UWB1
                    ...
                    for m in range(len(instances_UWB_current_file)):
                        ...
                        if mode==1:
                                ...
                                savingpath=data_path+'/'+'train'+'_dataset_str1_clf2'+'_'+timestep+'_filtred'+'/'+current_activity+'/'+'data_'+str(data_count)+'.pickle'
                            
                        if mode==2:
                            
                                savingpath=data_path+'/'+'val'+'_dataset_str1_clf2'+'_'+timestep+'_filtred'+'/'+current_activity+'/'+'data_'+str(data_count)+'.pickle'
                            
                        if mode==3:
                            
                                savingpath=data_path+'/'+'test'+'_dataset_str1_clf2'+'_'+timestep+'_filtred'+'/'+current_activity+'/'+'data_'+str(data_count)+'.pickle'    
